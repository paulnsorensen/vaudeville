...
- [x] US-011: Extract `_bucket_for_latency(lat) -> str` from `stats.py:106` — depth 3 → depth 1.
...
- [x] US-017: Fix `write_candidates` in `pool.py:143` — log warning and return early (don't overwrite) when existing YAML is non-dict. Add test for corrupt YAML scenario.
