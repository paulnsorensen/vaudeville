...
- [ ] US-005: Split `eval.py` (379→<300 lines) — extract `_build_parser`, `main`, and CLI entrypoint into `vaudeville/eval_cli.py`. Keep `classify_case`, `evaluate_rule`, `load_test_cases` in `eval.py`. Update `__main__.py` and `pyproject.toml` entry points. Fix all test imports.
...
- [ ] US-008: Shrink `eval.py:main` (50→<40 lines) — extract `_apply_extra_test_file(args, test_suites)` helper for the `if args.test_file and args.rule` block.
