...
class TestTierCeiling:
    ...
    def test_shadow_logs_without_action(
        ...
        monkeypatch.setenv("FAKE_KEY", "x")
        _write_rule(tmp_path, "shadow-rule", _decide_rule("shadow-rule", "block", "shadow"))
        fn, recorder = _decide_fn('{"outcome": "violation"}')
    ...
    def test_warn_caps_block(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setenv("FAKE_KEY", "x")
    ...
    def test_warn_caps_ask(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setenv("FAKE_KEY", "x")
    ...
    def test_warn_caps_rewrite(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setenv("FAKE_KEY", "x")
        _write_rule(
            tmp_path,
            "warn-rewrite",
            _decide_rule("warn-rewrite", "{action: rewrite, rule: rewrite-target}", "warn"),
        )
        _write_rule(
            tmp_path,
            "rewrite-target",
...
""",
    ...
    def test_disabled_rewrite_target_skips_model_call(
        ...
        _write_rule(
            tmp_path,
            "block-rewrite",
            _decide_rule("block-rewrite", "{action: rewrite, rule: rewrite-target}", "block"),
        )
        _write_rule(
            tmp_path,
            "rewrite-target",
...
""",
    ...
    def test_warn_caps_feedback(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setenv("FAKE_KEY", "x")
        _write_rule(tmp_path, "warn-feedback", _decide_rule("warn-feedback", "feedback", "warn"))
        fn, _ = _decide_fn('{"outcome": "violation"}')
    ...
    def test_warn_caps_escalate_result(
        ...
        _write_rule(
            tmp_path,
            "warn-escalate",
            _decide_rule("warn-escalate", "{action: escalate, rule: escalate-target}", "warn"),
        )
        _write_rule(
            tmp_path,
            "escalate-target",
...
""",
    ...
    def test_warn_keeps_add_context(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setenv("FAKE_KEY", "x")
        _write_rule(
            tmp_path,
            "warn-context",
            _decide_rule("warn-context", '{action: add-context, text: "extra context"}', "warn"),
        )
        fn, _ = _decide_fn('{"outcome": "violation"}')
    ...
    def test_warn_keeps_run(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setenv("FAKE_KEY", "x")
        _write_rule(
            tmp_path,
            "warn-run",
    ...
    def test_block_tier_denies(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setenv("FAKE_KEY", "x")
        _write_rule(tmp_path, "block-rule", _decide_rule("block-rule", "block", "block"))
        fn, _ = _decide_fn('{"outcome": "violation"}')
