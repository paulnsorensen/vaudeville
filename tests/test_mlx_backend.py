...
class TestMLXBackend:
    ...
    def _make_mocks(self, output: str = "VERDICT: clean") -> tuple:
        ...
        return mock_model, mock_tokenizer, mock_load, mock_stream_generate, mock_generate_step
    ...
    def test_classify_returns_generated_text(self) -> None:
        ...
        mock_mlx = MagicMock(
            load=mock_load, stream_generate=mock_stream_generate
        )
        mock_mlx_generate = MagicMock(generate_step=mock_generate_step)
        with patch.dict(
            "sys.modules",
            {"mlx_lm": mock_mlx, "mlx_lm.generate": mock_mlx_generate},
        ):
