...
class TestRunnerHelpers:
    ...
    def test_verdict_to_hook_response_log(self) -> None:
        runner = self._get_runner()
        result = runner.verdict_to_hook_response("test-rule", "{reason}", "test reason", "log")
        assert result == {}

    def test_verdict_to_hook_response_warn(self) -> None:
        runner = self._get_runner()
        result = runner.verdict_to_hook_response("test-rule", "{reason}", "test reason", "warn")
        assert result["decision"] == "warn"
        assert "Warning" in result["systemMessage"]

    def test_verdict_to_hook_response_block(self) -> None:
        runner = self._get_runner()
        result = runner.verdict_to_hook_response("test-rule", "Quality: {reason}", "hedging", "block")
        assert result["decision"] == "block"
