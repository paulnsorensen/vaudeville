...
def rules_search_path(project_root: str | None = None) -> list[str]:
    ...
    return [
        rules_dir for rules_dir, layer in rule_layers(project_root) if layer in _EDITABLE_LAYERS
    ]
...
def locate_all_rule_files(rule_name: str, project_root: str | None = None) -> list[Path]:
...
def set_tier(rule_name: str, new_tier: str, project_root: str | None = None) -> Path:
    ...
    new_content, count = re.subn(r"^tier:\s*\S+", f"tier: {new_tier}", content, flags=re.MULTILINE)
    if count == 0:
...
def list_rules_with_source(
    ...
    return [
        (entry.rule, str(Path(entry.path).parent))
...
def get_draft_rule_names(rules_dir: str) -> set[str]:
    ...
    names: set[str] = set()
    try:
        filenames = [p.name for p in Path(rules_dir).iterdir()]
    except OSError:
        return names
